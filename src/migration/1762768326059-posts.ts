...
export class Posts1762768326059 implements MigrationInterface {
    name = 'Posts1762768326059'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP INDEX \`email\` ON \`users\``);
        await queryRunner.query(`CREATE TABLE \`posts\` (\`id\` int NOT NULL AUTO_INCREMENT, \`n\` varchar(255) NOT NULL, \`e\` varchar(255) NOT NULL, \`s\` decimal(10,2) NOT NULL, \`e\` decimal(10,2) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`posts\` DROP COLUMN \`e\``);
        await queryRunner.query(`ALTER TABLE \`posts\` ADD \`e\` decimal(10,2) NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`posts\` DROP COLUMN \`e\``);
        await queryRunner.query(`ALTER TABLE \`posts\` ADD \`e\` varchar(255) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`role\``);
